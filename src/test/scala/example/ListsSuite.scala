...
class ListsSuite extends FunSuite {
  ...
  test("Does max of an empty list throws NoSuchElementException"){
  ...
  test("max of negative numbers"){
    assert(max(List(-2,-500, -1, -7)) === -1);
  }

  test("the max of a list with repeated elements"){
    assert(max(List(7,7,7,-2,-6,9,9,0)) === 9);
  }
}
